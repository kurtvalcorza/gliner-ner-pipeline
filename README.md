...
`tutorials/gliner_ner_colab.ipynb` is declared `TASK-INFERENCE` and is **standalone** (DIMER Notebook Specification 1.1 §3.6): it is generated by `tools/build_notebook.py` from `tools/notebook_template.py` and carries this package's `pipeline.py` verbatim, both pinned model identities, **both** snapshot manifests and the exact runtime pins, so the exported `.ipynb` keeps working without this repository being reachable — there is no clone, no repository install and no repository import on its primary path. Do not edit it by hand; change the package or the template and regenerate (`python tools/build_notebook.py`; `--check` is enforced by the validator and CI). Its default path authors a synthetic sentence and a four-type label set, surfaces `MAX_TEXT_CHARS`/`MAX_LABELS`/`MAX_LABEL_CHARS` and the threshold semantics, stages the missing GLiNER weight file with `stage_missing_files(..., allow_download=True)` and the encoder assets with `stage_missing_encoder_files(..., allow_download=True)`, digest-verifies both with `verify_snapshot`/`verify_encoder_snapshot`, validates the request through `validate_inputs` into `outputs/gliner_ner_input_manifest.json`, detects entities through the public API, writes `evaluation_report` to `outputs/gliner_ner_evaluation_report.json`, and exports the spans with offsets and scores plus provenance JSON carrying both identities. The report is `not-measurable` unless the learner pastes gold spans, in which case the repository's `entity_f1` is reported as `sample-sanity`. BYOD is optional and gated off by default. See `tutorials/README.md`.
