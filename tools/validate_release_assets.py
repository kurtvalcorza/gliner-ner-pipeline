...
EXPECTED_MODEL_ID = "urchade/gliner_multi-v2.1"
...
# so the expression also names the encoder directory (the template's `model_load` key).
MODEL_LOAD_EXPR = f"{PIPELINE_CLASS}.from_pretrained(weights_dir=WEIGHTS_DIR, encoder_dir=ENCODER_WEIGHTS_DIR)"
# Additional 40-hex revisions a document may legitimately cite: the pinned mDeBERTa-v3 encoder
# snapshot (tokenizer + config, no weights) this pipeline verifies alongside the GLiNER checkpoint.
KNOWN_SHAS: frozenset[str] = frozenset(("a0484667b22365f84929a935b5e50a51f71f159d",))
...
EXPECTED_OUTPUTS = (
    "outputs/gliner_ner_input_manifest.json",
    "outputs/gliner_ner_evaluation_report.json",
    "outputs/gliner_ner_result.json",
    "outputs/gliner_ner_entities.csv",
)
...
CODE_MARKERS = (
    ...
    # The second pinned snapshot (ST3/ST4): its manifest is carried inline, staged and digest-verified
    # through the carried module's own encoder helpers before the pipeline loads.
    "ENCODER_MANIFEST = {",
...
MARKDOWN_MARKERS = (
    "**Capability:** zero-shot named-entity recognition with a caller-supplied label set",
    "**No adaptation occurs:**",
    "**not a calibrated probability**",
    "**owned by the caller**",
    "**Two pinned snapshots**",
    "cuts input beyond 384 words",
    "`fix_mistral_regex`",
    ...
    "`MAX_TEXT_CHARS`",
    "`MAX_LABELS`",
    "entity linking or normalisation, relation extraction, coreference",
)
