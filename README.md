...
- Repository adaptation: **E2E** (bounded domain fine-tuning with frozen mDeBERTa-v3 backbone and portable adapter artifacts)
...
`tutorials/gliner_ner_colab.ipynb` is declared `E2E` and is **standalone** (DIMER Notebook Specification 2.0 §4): it is generated by `tools/build_notebook.py` from `tools/notebook_template.py` and embeds 3 package modules (`metrics.py`, `samples.py`, `pipeline.py`) verbatim in dependency order, both pinned model identities, **both** snapshot manifests and the exact runtime pins, so the exported `.ipynb` keeps working without this repository being reachable — there is no clone, no repository install and no repository import on its primary path. Do not edit it by hand; change the package or the template and regenerate (`python tools/build_notebook.py`; `--check` is enforced by the validator and CI). Its default path validates a deterministic 24-sentence biomedical NER dataset, splits disjoint train/validation subsets, stages the missing GLiNER weight file with `stage_missing_files(..., allow_download=True)` and the encoder assets with `stage_missing_encoder_files(..., allow_download=True)`, digest-verifies both with `verify_snapshot`/`verify_encoder_snapshot`, evaluates pre-adaptation zero-shot baseline metrics, runs bounded fine-tuning freezing the mDeBERTa backbone, evaluates post-adaptation exact-span micro/macro P/R/F1 deltas into `outputs/gliner_ner_evaluation_report.json`, performs inference on unseen clinical text, exports a portable adapter `.pt` artifact (`outputs/gliner_ner_adapter.pt`), verifies bit-level reload parity via `GLiNERPipeline.from_artifact`, and writes `outputs/gliner_ner_result.json` and `outputs/gliner_ner_entities.csv`. BYOD is optional and gated off by default. See `tutorials/README.md`.
